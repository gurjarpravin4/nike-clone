  ...
  return (
    <section id="home" className='w-full flex xl:flex-row flex-col justify-center min-h-screen gap-10 max-container'>
      <div className="relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28">
        ...
        <div className="flex justify-start flex-wrap w-full mt-10 gap-16">
          {statistics.map((statistic) => (
            <div key={statistic.label}>
              <p className='text-4xl font-palanquin font-bold'>{statistic.value}</p>
              <p className='leading-7 font-mono text-slate-gray'>{statistic.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className='relative flex-1 flex justify-center items-center xl:min-h-screen max-xl:py-40 bg-primary bg-hero bg-cover bg-center'>
        <img src={bigShoeImage} alt="bigShoe1" width={610} height={500} className='object-contain relative z-10' />
